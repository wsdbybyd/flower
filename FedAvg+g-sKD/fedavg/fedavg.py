...
class FedAvg(Strategy):
    ...
    该策略集成了两大核心功能模块：
    1. Ground-Assisted Dual Distillation (地面辅助双向蒸馏)*
       - Stage 1: 在分发前，利用地面站 Teacher 模型指导全局 Student 模型。
       - Stage 3: 在聚合后，利用更新后的全局 Student 模型反向更新 Teacher 模型。
    ...
    def __init__(
        ...
        # 1. 实例化 Teacher 模型 (Server-Side Large Model)
        self.teacher_model = TeacherNet()
        self.teacher_model.to(self.device)
        
        # 2. 加载地面站持有的校准数据集 (用于蒸馏)
        self.gs_dataloader = load_centralized_dataset()
        
        # Teacher 的学习率 (将在 ServerApp 中配置并注入)
    ...
    def summary(self) -> None:
        ...
        log(INFO, "\t    Ground-Assisted LEO FL with Dual Distillation Enabled.")
    ...
    def _update_link_state_from_replies(self, replies: list[Message]) -> None:
        """
        从客户端回传的 Metrics 中提取物理层链路状态 (SNR, Delay, Margin 等)，
        并更新本地缓存和成员状态。
        """
        for msg in replies:
    ...
    def configure_train(
        ...
        关键逻辑：
        1. 执行 Ground Station Distillation: 使用 Teacher 模型微调即将下发的全局模型。
        2. 根据通信窗口状态选择最佳卫星节点。
        3. 构造并发送训练指令 (含 APSKD 参数)。
        """
        ...
        # 论文 Section 2.2: 在分发前，利用 Teacher 指导 Global Student
        # ============================================================
        ...
        # 使用下发给 Client 的 lr 作为 Student 在地面微调的学习率
        ...
        # 封装蒸馏后的 Student 参数，准备分发
        distilled_arrays = ArrayRecord(student_model.state_dict())
        ...
        # 写入轮次信息，发送 distilled_arrays (Stage 1 优化后的参数)
        config["server-round"] = server_round
        record = RecordDict({self.arrayrecord_key: distilled_arrays, self.configrecord_key: config})
    ...
    def aggregate_train(
        ...
        关键逻辑：
        1. 接收卫星回传的模型参数 (经过 APSKD 训练)。
        2. 执行 FedAvg 聚合。
        3. 执行 Reverse Distillation: 用聚合后的模型反向更新地面站 Teacher。
        """
        ...
        # 论文 Section 2.4: 更新 Teacher 模型以吸收卫星学到的新知识
        # ============================================================
        ...
        # 执行反向蒸馏，更新 self.teacher_model
        # 通常只需少量 epoch (例如 1) 来传递知识，避免 Teacher 遗忘
        distill_student_to_teacher(
