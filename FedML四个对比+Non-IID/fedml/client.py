...
def train(msg: Message, context: Context) -> Message:
    ...
    if train_mode == "fedavg":
        # ---- FedAvg Baseline：标准 SGD，无 meta-learning / KD ----
        fedavg_lr            = float(_get(cfg, "learning-rate",          0.01))
        fedavg_epochs        = int(  _get(cfg, "fedavg-local-epochs",    1))
        fedavg_wd            = float(_get(cfg, "fedavg-weight-decay",    1e-4))
        fedavg_label_smooth  = float(_get(cfg, "fedavg-label-smoothing", 0.1))
        train_loss = train_fedavg_fn(
            model, trainloader, device,
            lr=fedavg_lr,
            epochs=fedavg_epochs,
            weight_decay=fedavg_wd,
            label_smoothing=fedavg_label_smooth,
        )
        ...
        # Mixed v5：FO-MAML outer-loop + APSKD inner-loop
        ...
        kd_warmup_rounds = int(_get(cfg, "kd-warmup-rounds", 50))

        # 跨轮次 snapshot：用本轮下发的全局模型作为 APSKD 的软标签教师
        # inner-loop 中 temp_model 随本地数据更新后与 snapshot 产生实质差距
        # KL 天然非零，APSKD 约束真正有效（v5 内部用 net 副本时 KL≈0 的问题）
