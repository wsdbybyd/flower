...
def make_global_evaluate(meta_adapt_steps: int, meta_adapt_lr: float) -> callable:
    def global_evaluate(server_round: int, arrays: ArrayRecord) -> MetricRecord:
        ...
        # Meta-Learning 需要执行 Adapt -> Test
        test_loss, test_acc = test_meta(
            model, test_dataloader, device,
            adaptation_steps=int(meta_adapt_steps), adaptation_lr=float(meta_adapt_lr),
        )
...
def main(grid: Grid, context: Context) -> None:
    ...
    for client_train_mode in modes_to_run:
        ...
        # 同时提取 evaluate_metrics_serverapp 和 train_metrics_clientapp
        ...
        if history_eval:
            ...
            for r in history_eval:
                ...
                metrics_to_save[int(r)] = {
                    ...
                    # 将客户端回传并聚合的 Train Loss 保存进 JSON
    ...
    plot_multi_mode_comparison()     # 绘制全局 Test Accuracy 对比
