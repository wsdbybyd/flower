...
class TeacherNet(nn.Module):
    ...
    这里给一个“更宽/更深一点”的 CNN 示例（你也可以替换成 ResNet 等）。
    """
...
def test_centralized_dataset():
    ...
    """
    from datasets import load_dataset  # 延迟导入，避免环境缺依赖时 import 崩溃
...
def load_representative_dataset(
    *,
    ...
    论文里这可以是全局子集/合成数据；这里用 CIFAR-10 的某个 split 子集模拟。
...
# -----------------------------------------------------------------------------
# 4. 双向蒸馏 (Bidirectional KD) + APSKD (按你PDF中的公式)
# -----------------------------------------------------------------------------
...
def kd_loss_lkd(
    ...
    *,
    ...
    注意：KL 使用 teacher 的 soft target（概率分布）做 target，student 用 log_softmax 输入 KLDivLoss。
    """
    ce = F.cross_entropy(z_student, y_true)

    t = float(temperature)
    # KLDivLoss 需要输入 log-prob 与 prob target
...
def reverse_distill_student_to_teacher(
    *,
    ...
    实现上：把 teacher 当“student”，把聚合后的 student 当“teacher”。
    """
...
def train_apskd(
    ...
    *,
    ...
    - 任务损失 Lt = CE(P^t(x), y)
    - 蒸馏损失 Ld = KL(P^t(x), P^{t-1}(x)) * Lt / (Lt + Lt_prev)   （根据PDF排版推断）
    - 总损失 LAPSKD = Lt + Ld

    说明：
    - 这里“上一 epoch 的任务损失 Lt_prev”用上一 epoch 的平均任务损失近似（实现上更稳）。
    - KL 使用 softmax(logits/temperature) 做分布。
    """
    ...
    for ep in range(int(epochs)):
        # 复制上一 epoch 模型作为 teacher
        prev_model = copy.deepcopy(model).to(device).eval()

        # 如果上一轮没有 Lt_prev，用当前 epoch 的一个默认值（避免权重爆炸）
