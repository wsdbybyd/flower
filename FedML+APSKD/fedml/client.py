...
from fedml.task import train_apskd as train_apskd_fn     # （可选）APSKD 自蒸馏训练
...
def compute_link_metrics(
    *,
    ...
    delta_rician: float,    # 瑞利/莱斯衰落因子（你原项目里用 delta 表示）
    ...
    保持与原项目一致的写法（Shannon 容量 + 大气衰减 + 路损）。
    """
    c = 299_792_458.0
    ...
    # 3) 天线增益线性化（如果 A_T/A_R 很大，按 dB 处理；否则按线性处理）
...
def train(msg: Message, context: Context) -> Message:
    ...
    默认：FO-MAML
    可选：APSKD（由 server 下发 client-train-mode="apskd" 开启）
    """
    ...
    # ---- Step 6: 回传 ----
    train_mode = str(_get(cfg, "client-train-mode", "fomaml")).strip().lower()
    train_mode_id = 1.0 if train_mode == "apskd" else 0.0
