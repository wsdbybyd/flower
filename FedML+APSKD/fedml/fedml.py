...
# --- 你的任务模块：学生模型 +（你上一条给的 task.py 里新增的 KD 相关函数） ---
...
class FedMeta(Strategy):
    ...
    def __init__(
        ...
        # --- 双向蒸馏参数（也可不从这里传，直接放进 ConfigRecord 里） ---
        ...
        self._kd_device_pref = kd_device  # None / "cpu" / "cuda"

        self.teacher_model: Optional[TeacherNet] = None
        self._dcal_loader = None
        self._dglobal_loader = None

        # 记录本轮 forward KD loss，方便写入 aggregate_train 的 metrics
    ...
    def _kd_read_overrides(self, cfg: ConfigRecord) -> None:
        """允许从 ConfigRecord 覆盖（便于你在 pyproject/server 里配 kd-* 参数）"""
    ...
    def _ensure_kd_objects(self) -> None:
        """延迟初始化 teacher 与 D_cal/D_global（避免不启用 KD 时触发 datasets 依赖）"""
    ...
    def configure_train(self, server_round: int, arrays: ArrayRecord, config: ConfigRecord, grid: Grid) -> Iterable[Message]:
        ...
        # 允许从 config 中读取/覆盖 KD 参数
        self._kd_read_overrides(config)

        # ============================================================
        # （新增）Ground→Satellite 正向蒸馏：teacher -> student（下发前）
        # ============================================================
    ...
    def aggregate_train(self, server_round: int, replies: Iterable[Message]) -> tuple[ArrayRecord | None, MetricRecord | None]:
        ...
        # 2) 聚合客户端更新（FO-MAML/Reptile 形式：聚合更新后的参数）
        ...
        # 3) （新增）Student→Teacher 反向蒸馏：用聚合 student 更新 teacher
        if self._kd_enable:
            try:
                # 注意：这里用 “训练 config 的 kd 参数” 通常就够；若你把 kd 参数只配在 train_config 里，这里仍有效
                # 但 aggregate_train 没有 config，所以用 self 内部缓存即可
                self._ensure_kd_objects()
                device = self._kd_device()
                ...
                # 把 KD 的指标写进本轮训练 metrics 里（便于画图/分析）
