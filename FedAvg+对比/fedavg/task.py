...
class BigTeacherNet(nn.Module):
    """
    [教师模型] 大型 CNN (此前可能叫 TeacherNet，统一为 BigTeacherNet)
...
def load_centralized_dataset():
    """Server 端加载部分数据集 (用于 FL 过程中的蒸馏校准)"""
    # 兼容旧接口，复用上面的函数或单独定义
...
def distill_teacher_to_student(teacher_net, student_net, dataloader, device, epochs, lr, alpha, temp):
    ...
    # 逻辑其实与 distill_centralized 几乎一致，保留此别名以匹配 fedavg.py 调用
...
def distill_student_to_teacher(student_net, teacher_net, dataloader, device, epochs, lr):
    ...
    for _ in range(epochs):
        for batch in dataloader:
            ...
            # 反向只用 KL Loss，让 Teacher 学习 Student 的新分布
            loss = criterion_kl(
                F.log_softmax(logits_t, dim=1), # 注意这里没有除以 Temperature，通常反向微调可简化
...
def train_apskd(net, trainloader, epochs, lr, device, a_T):
    ...
    return 0.0 # 简化返回，实际使用中可计算 avg loss
