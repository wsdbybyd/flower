...
def plot_comparison(results_dict: dict, filename: str = "comparison_three_way.png"):
    ...
    # 按顺序遍历 (Baseline -> Only Ground -> Dual)
...
def run_strategy_session(
    ...
    context: Context, 
    session_name: str,
    enable_dual_distill: bool, # 是否开启每轮的双向蒸馏
    apskd_alpha: float         # 客户端自蒸馏强度 (0=关闭)
):
...
def main(grid: Grid, context: Context) -> None: 
    ...
    # 实验 B: Only Ground Distillation (仅热启动)
    # 特征: 蒸馏初始化 + 普通训练 (验证初始化的作用)
    res_only_ground = run_strategy_session(
        grid=grid, context=context,
        session_name="2. Ground Distillation",
        initial_model=distilled_model,  # <--- 差异点：使用蒸馏后的模型
        enable_dual_distill=False,      # <--- 差异点：关闭持续的联邦蒸馏
        apskd_alpha=0.0                 # <--- 差异点：关闭客户端自蒸馏
    )

    # 实验 C: Dual Distillation (完整方案)
    # 特征: 蒸馏初始化 + 双向联邦蒸馏 + APSKD
    target_apskd = float(context.run_config.get("apskd-a-T", 0.5))
    res_dual = run_strategy_session(
        grid=grid, context=context,
        session_name="3. Dual Distillation",
        initial_model=distilled_model,  # <--- 使用蒸馏后的模型
        enable_dual_distill=True,       # <--- 开启每轮交互
        apskd_alpha=target_apskd        # <--- 开启客户端自蒸馏
    )
...
def global_evaluate(server_round: int, arrays: ArrayRecord) -> MetricRecord:  
    ...
    # 注意：在 task.py 中定义 test_centralized_dataset 更好，避免混淆
    # 这里假设 task.py 里有 test_centralized_dataset() 辅助函数
    try:
