...
def train(msg: Message, context: Context) -> Message:
    ...
    # E2 diagnostics: Mixed warmup weight used in this round (None for non-mixed modes)
    lambda_kd_used = None
    ...
    if train_mode == "apskd":
        ...
        # E2: make warmup effective by passing server round to the mixed trainer
        current_round = int(_get(cfg, "server-round", 1))
        warmup_rounds = int(_get(cfg, "kd-warmup-rounds", 50))
        lambda_kd_used = min(0.2, float(current_round) / float(max(1, warmup_rounds)))
    ...
    if lambda_kd_used is not None:
        log(
            INFO,
            "[node %s] train_mode=%s loss=%.4f lambda_kd=%.4f (round=%d/%d) d_km=%.1f comm_ok=%s margin=%.2f",
            partition_id,
            train_mode,
            float(train_loss),
            float(lambda_kd_used),
            int(current_round),
            int(warmup_rounds),
            comm_metrics["d_km"],
    ...
    metrics = {
        ...
        "lambda_kd": float(lambda_kd_used) if lambda_kd_used is not None else -1.0,
        "server_round": float(current_round) if lambda_kd_used is not None else -1.0,
        "kd_warmup_rounds": float(warmup_rounds) if lambda_kd_used is not None else -1.0,
