...
class FedMeta(Strategy):
    ...
    def __init__(
        ...
        kd_reverse_enable: bool = True,
        kd_reverse_freeze_bn: bool = False,
        ...
        self._kd_reverse_enable = bool(kd_reverse_enable)
        self._kd_reverse_freeze_bn = bool(kd_reverse_freeze_bn)
    ...
    def _kd_read_overrides(self, cfg: ConfigRecord) -> None:
        ...
        self._kd_reverse_enable = _cfg_bool(cfg, "kd-reverse-enable", self._kd_reverse_enable)
        self._kd_reverse_freeze_bn = _cfg_bool(cfg, "kd-reverse-freeze-bn", self._kd_reverse_freeze_bn)
    ...
    def aggregate_train(self, server_round: int, replies: Iterable[Message]) -> tuple[ArrayRecord | None, MetricRecord | None]:
        ...
        # 3) Student→Teacher 反向蒸馏：用聚合 student 更新 teacher
        if self._kd_enable and ((not self._kd_reverse_enable) or (self._kd_reverse_epochs <= 0)):
            if metrics_out is None:
                metrics_out = MetricRecord({})
            metrics_out["kd_reverse_disabled"] = 1.0

        elif self._kd_enable:
            try:
                self._ensure_kd_objects()
                device = self._kd_device()
                ...
                rev_loss = reverse_distill_student_to_teacher(
                    ...
                    freeze_bn=self._kd_reverse_freeze_bn,
                )
