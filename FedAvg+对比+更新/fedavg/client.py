...
from fedavg.task import train_apskd, train_plain_sgd
...
def compute_link_metrics(*, model_bits: int, d_km: float, tau_d_s: float, A_T: float, A_R: float,
                         f_c_hz: float, G_H: float, delta_rician: float, psi_db_per_km: float,
                         zeta_km: float, w_u_hz: float, p_u_w: float, w_d_hz: float,
                         p_d_w: float, sigma2: float) -> Dict[str, float]:
    c = 299_792_458.0
    ...
    return {
        "d_km": float(d_km), "tau_d_s": float(tau_d_s), "rho": float(rho), "A_atm": float(A_atm),
        "snr_u": float(snr_u), "snr_d": float(snr_d), "F_u_bps": float(F_u), "F_d_bps": float(F_d),
        "T_u_s": float(T_u), "T_d_s": float(T_d), "T_total_s": float(T_total),
        "window_margin_s": float(float(tau_d_s) - T_total) if math.isfinite(T_total) else float("-inf"),
        "comm_ok": 1.0 if comm_ok else 0.0, "model_bits": float(model_bits),
    }
...
def _choose_train_mode(cfg: Dict[str, Any]) -> tuple[str, float]:
    apskd_a_T = float(_get(cfg, "apskd-a-T", 0.0))
    local_train_mode = str(_get(cfg, "local-train-mode", "auto")).lower()
    if local_train_mode in {"fedavg", "plain_sgd", "clean_fedavg", "baseline"}:
        return "fedavg", apskd_a_T
    if local_train_mode == "apskd":
        return "apskd", apskd_a_T
    if apskd_a_T > 0.0:
        return "apskd", apskd_a_T
    return "fedavg", apskd_a_T
...
def train(msg: Message, context: Context) -> Message:
    ...
    mode, apskd_a_T = _choose_train_mode(cfg)

    if mode == "fedavg":
        train_loss = train_plain_sgd(model, trainloader, local_epochs, lr, device)
    else:
        train_loss = train_apskd(model, trainloader, local_epochs, lr, device, a_T=apskd_a_T)
    ...
    comm_metrics = compute_link_metrics(
        model_bits=model_bits, d_km=d_km, tau_d_s=tau_d_s, A_T=A_T, A_R=A_R, f_c_hz=f_c_hz,
        G_H=G_H, delta_rician=delta_rician, psi_db_per_km=psi_db_per_km, zeta_km=zeta_km,
        w_u_hz=w_u_hz, p_u_w=p_u_w, w_d_hz=w_d_hz, p_d_w=p_d_w, sigma2=sigma2,
    )

    model_record = ArrayRecord(model.state_dict())
    metrics = {
        "train_loss": float(train_loss), "num-examples": len(trainloader.dataset),
        "tau_d_s": comm_metrics["tau_d_s"], "T_total_s": comm_metrics["T_total_s"],
        "window_margin_s": comm_metrics["window_margin_s"], "comm_ok": comm_metrics["comm_ok"],
        "d_km": comm_metrics["d_km"], "F_u_bps": comm_metrics["F_u_bps"],
        "F_d_bps": comm_metrics["F_d_bps"], "A_atm": comm_metrics["A_atm"],
        "snr_u": comm_metrics["snr_u"], "snr_d": comm_metrics["snr_d"],
        "rho": comm_metrics["rho"], "model_bits": comm_metrics["model_bits"],
    }
...
def evaluate(msg: Message, context: Context) -> Message:
    ...
    metrics = {
        "eval_loss": float(eval_loss), "eval_acc": float(eval_acc), "num-examples": len(valloader.dataset),
        "tau_d_s": comm_metrics["tau_d_s"], "T_total_s": comm_metrics["T_total_s"],
        "window_margin_s": comm_metrics["window_margin_s"], "comm_ok": comm_metrics["comm_ok"],
        "d_km": comm_metrics["d_km"], "A_atm": comm_metrics["A_atm"], "snr_u": comm_metrics["snr_u"],
        "snr_d": comm_metrics["snr_d"], "rho": comm_metrics["rho"], "model_bits": comm_metrics["model_bits"],
    }
    metric_record = MetricRecord(metrics)
