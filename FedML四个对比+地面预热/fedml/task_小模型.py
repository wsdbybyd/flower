...
def train_fomaml_apskd(net, trainloader, device, alpha, beta, num_inner_steps,
    ...
    eps = 1e-12
    t = float(temperature)
    lambda_apskd = 1        # APSKD 步的学习率缩放系数（相对 beta）
...
def train_fedavg(net, trainloader, device, lr: float = 0.01, epochs: int = 1):
    """
    标准 FedAvg 本地训练 (McMahan et al., 2017)。
    纯 SGD + CrossEntropyLoss，无 meta-learning / 知识蒸馏，作为 baseline。
    """
    net.to(device).train()
    criterion = nn.CrossEntropyLoss().to(device)
    optimizer = torch.optim.SGD(net.parameters(), lr=float(lr), momentum=0.9)
