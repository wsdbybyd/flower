...
def evaluate(msg: Message, context: Context) -> Message:
    ...
    # ---- Step 4: 执行评估 ----
    if train_mode == "fedavg":
        # FedAvg baseline：zero-shot 评估，与训练目标（直接分类）一致
        # 不做 meta-adaptation，避免 adaptation 步骤破坏已收敛的 FedAvg 参数
        from fedml.task import test as test_fn
        eval_loss, eval_acc = test_fn(model, valloader, device)
    else:
        # FedMeta 系列：Adapt -> Test，衡量快速适应能力
